import type { MapPc, StaticMapData, StaticMapZone } from "../shared/types";
import { createPcTechnicalDetails } from "../pc-details/logic/pcTechnicalDetails";

/** Raw static dataset shape before technical details are generated. */
interface RawStaticMapData {
  image: StaticMapData["image"];
  zones: Array<
    Omit<StaticMapZone, "pcs"> & {
      pcs: Array<Pick<MapPc, "id" | "x" | "y">>;
    }
  >;
}
...
/** Seed data used to initialise the interactive map in the frontend. */
...
/** Normalised dataset consumed by the interactive map state. */
...
/**
 * Enriches one raw zone with generated technical details for each PC.
 *
 * @param zone Raw static zone.
 * @returns Zone ready to be consumed by the frontend.
 */
function createStaticZone(zone: RawZone): StaticMapZone {
  return {
    ...zone,
...
/**
 * Generates a complete PC record from a lightweight seed definition.
 *
 * @param pc Minimal static PC definition.
 * @param zoneId Zone owning the PC.
 * @returns Enriched PC with technical metadata.
 */
function enrichPcWithTechnicalDetails(
  pc: Pick<MapPc, "id" | "x" | "y">,
  zoneId: number,
): MapPc {
  return {
    ...pc,
